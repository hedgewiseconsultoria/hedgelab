...
export default function HedgeDashboard() {
  ...
  const receiveGuidedExposure = useCallback((publication: GuidedExposurePublication) => {
    setExposures(current => [{ exposure_id: publication.exposureId, description: publication.description, currency: publication.currency, direction: publication.direction, notional: publication.amount, cashflow_date: publication.maturityDate, created_at_utc: new Date().toISOString() }, ...current.filter(row => row.exposure_id !== publication.exposureId)]);
  }, []);
  useEffect(() => {
