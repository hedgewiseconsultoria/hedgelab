...
import HedgeDiagnosisCard from "@/components/HedgeDiagnosisCard";
...
export default function HedgeDashboard() {
  ...
  const [ptaxDate, setPtaxDate] = useState(today);
  const [ipcaPeriod, setIpcaPeriod] = useState("202607");
  ...
  const igpm = trpc.marketData.igpmPublishedTable.useQuery({ sourceUrl: FGV_IGPM_2026, year: 2026 }, { enabled: false, retry: false });
  ...
  const receiveCanonicalDiagnosis = useCallback((dataframes: CanonicalHedgeDataframes) => {
    setCanonicalDataframes(attachCanonicalFxSwapSizing(attachCanonicalRateSwapSizing(attachCanonicalNdfSizing(dataframes, { instrumentMasterRows, hedgeRows }), { instrumentMasterRows, hedgeRows }), { instrumentMasterRows, hedgeRows }));
  ...
  function addExposure() {
    ...
    setExposures(current => [...current, { exposure_id: crypto.randomUUID(), description: description.trim(), currency, direction, notional: parsedNotional, cashflow_date: cashflowDate, created_at_utc: new Date().toISOString() }]);
  ...
  async function exportExposureCsvArtifact() {
    try {
      ...
      const artifact = await createExposureCsvArtifact(exposures, sourceLineage as Array<Record<string, unknown>>);
  ...
  return <section className="mx-auto max-w-[1540px]">
    <div className="grid gap-5 xl:grid-cols-[1.5fr_.78fr]">
    ...
    <B3ManualCollectionCard onLineage={setManualB3Lineage} onNormalizations={setB3NormalizedArtifacts} />
    ...
    <SelicOverAccumulationCard onSnapshot={recordSessionSnapshot} />

    <ExposureMaturityBucketsCard exposures={exposures} />

    <CdiDebtCoverageSummaryCard dataframes={canonicalDataframes} />

    <HedgeDiagnosisCard onCanonicalDataframes={receiveCanonicalDiagnosis} />
