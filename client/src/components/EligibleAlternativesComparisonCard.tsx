...
export default function EligibleAlternativesComparisonCard({ dataframes, lineage = [], selectedAlternativeId, onSelectAlternative, diCurve, onDiCurve, diCurveAsOf = "2026-08-13", onOpenSimulation, coveragePct = 100, onCoverageChange = () => {}, b3Catalog = [] }: Props) {
  ...
  const selectedCatalogObservation = selectedAlternative && selectedSituation ? (() => { const family = catalogFamily(selectedAlternative, selectedSituation); const wantedType = selectedAlternative.alternative_kind.includes("OPTION") ? "OPTION" : "FUTURE"; return family ? b3Catalog.find(item => item.family === family && item.horizonDate === selectedSituation.horizon_date && (!item.instrumentType || item.instrumentType === wantedType))?.observations[0] ?? null : null; })() : null;
  void onDiCurve;
  void diCurveAsOf;
