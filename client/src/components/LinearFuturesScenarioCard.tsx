...
import FutureProtectionChart from "@/components/FutureProtectionChart";
...
export default function LinearFuturesScenarioCard({ onSnapshot, situation, alternative, observedPrice = null, observedSourceAsOf = null, observedSourceFile = null, observedSourceHashSha256 = null, coveragePct = 100 }: Props) {
  ...
  return <Card className="mt-7 rounded-2xl border-[#d7e7e2] bg-white shadow-none">
    ...
    {scenario.data && <><div className="mt-5"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Exposição sem hedge" value={format(scenario.data.unhedgedEconomicResult)} tone="ink" /><Metric label="Futuro" value={format(scenario.data.futuresResult)} tone="mint" /><Metric label="Residual combinado" value={format(scenario.data.residualResult)} tone="ink" /><Metric label="Razão efetiva" value={`${format(scenario.data.hedgeCoverageRatio * 100)}%`} tone="ink" /></div><p className="mt-3 text-[10px] leading-4 text-[#6a827d]">{scenario.data.limitations.join(" ")}</p></div><FutureProtectionChart instrumentLabel={instrumentLabel} horizonDate={horizonDate} economicDirection={economicDirection} hedgePosition={hedgePosition} exposureQuantity={input.exposureQuantity} hedgeContracts={input.hedgeContracts} contractUnitQuantity={input.contractUnitQuantity} initialPrice={input.initialPrice} scenarioPrice={input.scenarioPrice} quotationUnit={input.quotationUnit} /></>}
    </CardContent>
  </Card>;
}
