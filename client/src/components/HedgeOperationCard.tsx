...
export default function HedgeOperationCard({ situation, alternative, coveragePct, onCoverageChange, onOpenSimulation, diCurve, commodityMarketStatus = "idle", commodityMarketObservation, onLinkCommodityMarket, catalogObservation = null, compact = false }: Props) {
  ...
  const [simulatorMarginText, setSimulatorMarginText] = React.useState("");
  const simulatorMarginResult = parseNumber(simulatorMarginText);
  ...
  const sizing = calculateHedgeOperationSizing({ situation, alternative, coveragePct, observation: catalogObservation, marginTheoreticalMax: catalogObservation?.marginTheoreticalMax ?? null, marginSimulatorResult: simulatorMarginResult });
  ...
  const marginDetail = sizing.marginStatus === "official_simulator_result" ? "Resultado total informado para a carteira simulada" : isB3 ? "Informe o resultado total do simulador B3 para esta posição" : "Garantia depende dos termos e da contraparte";
  ...
  return <Card className={`mt-5 overflow-hidden rounded-2xl border-[#b9dfd4] bg-white shadow-[0_18px_38px_-32px_rgba(19,91,75,.6)] ${compact ? "" : ""}`}>
    ...
    <CardContent className="p-5"><div className="grid gap-3 rounded-xl border border-[#d8ebe5] bg-[#fbfefd] p-4 md:grid-cols-2 xl:grid-cols-4"><Metric label="Exposição transferida" value={quantity(situation)} detail={situation.description} /><Metric label="Horizonte" value={situation.horizon_date} detail="Vencimento declarado" /><Metric label="Posição econômica" value={alternative.hedge_direction === "BUY" ? "Comprada" : "Vendida"} detail={economicPosition(alternative)} /><Metric label="Cobertura-alvo" value={`${coveragePct}%`} detail="Hipótese de proteção, não quantidade contratada" /></div>
      ...
      {isB3 && <div className="mt-4 rounded-xl border border-[#cfe5df] bg-[#f8fcfa] p-4"><p className="text-[10px] font-semibold uppercase tracking-[.1em] text-[#527a72]">Resultado do simulador oficial B3 (opcional)</p><p className="mt-1 text-xs leading-5 text-[#607c76]">Cole aqui a margem total retornada pelo <a className="font-semibold text-[#1b6258] underline" href="https://simulador.b3.com.br/" target="_blank" rel="noreferrer">simulador.b3.com.br</a> para a mesma série, quantidade, direção, preço e data. O valor substitui a MT unitária e não é recalculado pelo Hedge Lab.</p><div className="mt-3 max-w-xs"><Input inputMode="decimal" value={simulatorMarginText} onChange={event => setSimulatorMarginText(event.target.value)} placeholder="Ex.: 77.824,13" aria-label="Margem total do simulador B3" /></div></div>}
