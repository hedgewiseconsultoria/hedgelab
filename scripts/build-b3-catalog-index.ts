...
import { readLocalB3MarginRows } from "../server/ingestion/b3MarginSnapshot";
...
const marginArchiveFilename = `MT${asOf.slice(2).replace(/-/g, "")}.zip`;
const margin = await readLocalB3MarginRows({ root: resolve(outputRoot, ".."), asOf, archiveFilename: marginArchiveFilename });
const catalog = {
  ...
  marginRows: margin?.rows.filter(marginRow => market.dataframe.some(observation => observation.instrumentId === marginRow.instrumentId)) ?? [],
  ...
  lineage: {
    ...
    ...(margin ? { margin: { sourceAsOf: asOf, officialDownloadUrl: `https://www.b3.com.br/pesquisapregao/download?filelist=${marginArchiveFilename},`, archive: { filename: margin.archiveFilename, sha256: margin.archiveHashSha256 }, csv: { sourceFile: margin.csvFilename } } } : {}),
  },
};
...
console.log(JSON.stringify({ asOf, rows: catalog.rows.length, marginRows: catalog.marginRows.length, bytes: bytes.length, associationStatus: catalog.associationStatus, coverage: catalog.coverage }, null, 2));
