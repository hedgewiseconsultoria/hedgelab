...
export const canonicalB3ObservationLinkRowSchema = z.object({
  ...
  price_source: z.object({ report_type: z.enum(["BVBG.086.01", "BVBG.187.01"]), source_url: z.string().url(), source_file: z.string().min(1), source_asof: isoDateSchema, source_hash_sha256: z.string().regex(/^[a-f0-9]{64}$/i), normalized_csv_storage_key: z.string().min(1).nullable(), normalized_csv_sha256: z.string().regex(/^[a-f0-9]{64}$/i).nullable(), normalized_manifest_storage_key: z.string().min(1).nullable() }),
  instrument_source: z.object({ source_url: z.string().url(), source_file: z.string().min(1), source_asof: isoDateSchema, source_hash_sha256: z.string().regex(/^[a-f0-9]{64}$/i), normalized_csv_storage_key: z.string().min(1).nullable(), normalized_csv_sha256: z.string().regex(/^[a-f0-9]{64}$/i).nullable(), normalized_manifest_storage_key: z.string().min(1).nullable() }),
