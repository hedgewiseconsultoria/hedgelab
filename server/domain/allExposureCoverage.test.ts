...
describe("cobertura de todos os tipos de exposição", () => {
  ...
  it("usa o prêmio da opção e não o ajuste do futuro quando ambos estão presentes", () => {
    const result = calculateHedgeOperationSizing({ situation: situation({ declared_quantity: 1_500_000 }), alternative: alternative("B3_DOL_OPTION"), coveragePct: 100, observation: observation({ symbol: "DOLV26C005350", instrumentType: "OPTION", optionType: "CALL", exercisePrice: 5.35, adjustedQuote: 5_208.75, lastPrice: 28.38 }) });
    expect(result.observedPrice).toBe(28.38);
    expect(result.premiumValue).toBe(42_570);
    expect(result.priceLabel).toMatch(/prêmio/i);
  });
