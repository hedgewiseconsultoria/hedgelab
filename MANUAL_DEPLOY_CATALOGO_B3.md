...
A margem operacional dos futuros B3, inclusive DI1, é consultada automaticamente pelo navegador no endpoint CORS público do simulador oficial B3 para a mesma série, quantidade, direção, preço e data da operação. O resultado `riskWithoutCollateral` preenche a margem total da carteira; a MT B3 permanece como referência técnica e não é multiplicada automaticamente pela quantidade. Se o simulador não responder, a margem fica bloqueada e o motivo é exibido.

A interface diferencia três situações. **Efetivo** significa que existe observação oficial compatível e preço/ajuste observável. **Parametrizado** significa que a alternativa pode ser estudada com termos declarados, como NDF e swap, sem que o sistema invente preço, taxa ou contrato. **Bloqueado** significa que a fonte, o contrato, a associação, o preço obrigatório ou a resposta do simulador de margem estão ausentes; nesse caso a aplicação mostra a razão e não substitui a falta por um valor inventado. A MT B3 permanece como referência técnica; a margem da carteira é o total retornado automaticamente pelo simulador oficial B3 para a mesma posição.

## Validação feita antes da entrega

A correção foi verificada localmente com o check de TypeScript, a suíte completa e o build de produção. A suíte final registrou **331 testes aprovados e 6 ignorados**; o build terminou com sucesso, mantendo apenas os avisos não bloqueantes já existentes sobre Analytics opcional e tamanho do bundle. Também foi validado o caso de opção DOL `DOLV26C005350`: prêmio observado de `R$ 28,38`, 30 contratos para uma exposição de USD 1,5 milhão e custo total de `R$ 42.570`, sem a multiplicação indevida por 1.000. O strike exibido é normalizado para `R$ 5,35` quando a fonte cadastral entrega `5350` em escala milhar. A margem automática foi validada no navegador contra o endpoint público do simulador B3, que retornou HTTP 200 e `riskWithoutCollateral` para uma carteira WDO de teste.
